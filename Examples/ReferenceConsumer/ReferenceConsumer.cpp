...
public:
    ...
    void onStateChanged(const NumericMetricState& p_changedState) override
    {
        ...
        DebugOut(DebugOut::Default, "ReferenceConsumer")
            << "Consumer: Received value changed of " << getDescriptorHandle() << ": " << t_newValue << std::endl;
    ...
    {
        DebugOut(DebugOut::Default, "ReferenceConsumer")
            << "Consumer: Received operation invoked (numeric metric) (ID, STATE) of " << getDescriptorHandle() << ": " << oic.transactionId
...
public:
    ...
    void onStateChanged(const AlertSignalState& p_changedState) override
    {
        if(p_changedState.hasPresence())
            DebugOut(DebugOut::Default, "ReferenceConsumer")
                << getDescriptorHandle() << " alert presence changed to: " << EnumToString::convert(p_changedState.getPresence()) << "\n";
        {
            auto t_newValue = p_changedState.getPresence();
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if(t_consumer != nullptr)
        {
            ...
            auto test7and8Evaluation = std::async(std::launch::async,
                                                  [numeric_dynamic_get,
                                                   alert_condition_mds,
                                                   alert_condition_vmd,
                                                   alert_signal_vmd,
                                                   alert_signal_mds] {
                                                      ...
                                                      numeric_dynamic_get->getTimesValueChanged() > 4 ?
                                                      ...
                                                      ((alert_condition_mds->getTimesValueChanged() > 4)
                                                       ...
                                                       && (alert_signal_mds->getTimesValueChanged() > 4)) ?
            ...
            if(t_consumer->registerStateEventHandler(numeric_dynamic_get.get())
               ...
               && t_consumer->registerStateEventHandler(alert_condition_vmd.get()) && t_consumer->registerStateEventHandler(stream.get()))
            {
            ...
            if(numeric_static_get)
            {
                if(numeric_static_get->hasMetricValue())
                {
                    Util::DebugOut(Util::DebugOut::Default, "ReferenceConsumer")
                        << "Requested get metrics value: " << numeric_static_get->getMetricValue().getValue();
                }
            }
            ...
            if(string_static_get)
            {
                ...
                {
                    Util::DebugOut(Util::DebugOut::Default, "ReferenceConsumer")
                        << "Requested get metrics value: " << string_static_get->getMetricValue().getValue();
                }
            }
            ...
            if(enum_static_get)
            {
                ...
                {
                    Util::DebugOut(Util::DebugOut::Default, "ReferenceConsumer")
                        << "Requested get metrics value: " << enum_static_get->getMetricValue().getValue();
                }
            }
                    ...
                    if(vmd.hasSco())
                    {
                        ...
                        for(const auto& operation : setOperations)
                        {
                            if(operation.getOperationTarget() == HANDLE_SET_ESTRM)
                            {
                                ...
                                // Use the returned state, set the value and "commit" it
                                t_setEnumMetricState->setMetricValue(
                                    StringMetricValue{MetricQuality{MeasurementValidity::Vld}}.setValue(t_newValue));
                                // Now
                                FutureInvocationState fis;
