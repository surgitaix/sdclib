...
const std::string DEVICE_EPR("UDI-1234567890-e30");
...
const std::string HANDLE_CLOCK{ "CL.mds0" };
...
public:
	...
	NumericMetricState createState(double p_initialValue)
	{
		...
		t_newState.setMetricValue(NumericMetricValue(MetricQuality(MeasurementValidity::Vld)).setValue(p_initialValue))
	...
	InvocationState onStateChangeRequest(const NumericMetricState& p_state, const OperationInvocationContext& p_oic) override
	{
		...
		else
		{
			...
			DebugOut(DebugOut::Default, "ReferenceProvider")
				<< "Provider: handle_set received state change request. State's value: " << p_state.getMetricValue().getValue()
				<< std::endl;
			// extract information from the incoming operation
			DebugOut(DebugOut::Default, "ReferenceProvider") << "Operation invoked. Handle: " << p_oic.operationHandle << std::endl;
...
class ClockStateHandler : public SDCProviderComponentStateHandler<ClockState>
{
public:
	ClockStateHandler(const std::string p_descriptorHandle)
		: SDCProviderComponentStateHandler(p_descriptorHandle)
	{
	}

	ClockState getInitialState()
	{
		ClockState newState{ getDescriptorHandle(), false };
		return newState;
	}
};
...
public:
	...
	{
		m_sdcProvider.setEndpointReference(DEVICE_EPR);
		...
		m_sdcProvider.addMdStateHandler(&m_clockStateHandler);
	}
	...
	const std::string getStringRepresentationOfMDIB()
	{
		const auto MDIB = m_sdcProvider.getMdib();
...
private:
	...
	ClockStateHandler m_clockStateHandler{ HANDLE_CLOCK };
...
int main(int argc, char* argv[])
{
	...
	if (!t_SDCInstance->bindToInterface("{5576B12E-A7F6-4BB0-9B85-3A2CFE11F53E}", true))
	{
	...
	std::cout << t_provider.getStringRepresentationOfMDIB() << std::endl;
