		...
		if (t_response != nullptr) {
			for (const auto & t_metadata : t_response->getMetadataSection()) {
				if (t_metadata.getDialect() != OSELib::WS_MEX_DIALECT_REL
					|| !t_metadata.getRelationship().present()
					|| t_metadata.getRelationship().get().getHosted().empty()
					) {
					continue;
				}
		...
		if (t_response_metadata != nullptr) {
			...
			for (const auto & t_metadata_iter : t_response_metadata->getMetadataSection()) {
				if (t_metadata_iter.getDialect() != SDC::WS_MEX_DIALECT_STREAM
					...
					|| t_metadata_iter.getStreamDescriptions().get().getStreamType().empty()
					|| !t_metadata_iter.getStreamDescriptions().get().getStreamType().front().getStreamTransmission().getStreamAddress().present()
					) {
					continue;
				}
				t_deviceDescription->addStreamMulticastAddressURI(Poco::URI(t_metadata_iter.getStreamDescriptions().get().getStreamType().front().getStreamTransmission().getStreamAddress().get()));
			}
		}
