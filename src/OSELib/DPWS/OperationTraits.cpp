...
std::unique_ptr<RenewTraits::Response>
    RenewTraits::dispatch(RenewTraits::Dispatcher& p_dispatcher, const Request& p_request, const RequestIdentifier& p_identifier)
{
...
std::unique_ptr<GetStatusTraits::Response>
    GetStatusTraits::dispatch(GetStatusTraits::Dispatcher& p_dispatcher, const Request& p_request, const RequestIdentifier& p_identifier)
{
    return p_dispatcher.dispatch(p_request, p_identifier);
}
...
std::unique_ptr<UnsubscribeTraits::Response> UnsubscribeTraits::dispatch(UnsubscribeTraits::Dispatcher& p_dispatcher,
                                                                         const Request& p_request,
                                                                         const RequestIdentifier& p_identifier)
{
    return p_dispatcher.dispatch(p_request, p_identifier);
}


xml_schema::Qname SubscriptionEndTraits::MessageType()
{
    return xml_schema::Qname(WS_NS_EVENTING, "SubscriptionEnd");
}
std::string SubscriptionEndTraits::NotificationName()
{
    return WS_ACTION_SUBSCRIPTIONEND;
}
std::string SubscriptionEndTraits::Action()
{
    return "http://schemas.xmlsoap.org/ws/2004/08/eventing/SubscriptionEnd";
}
