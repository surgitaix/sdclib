...
void SubscriptionClient::run() {
	...
	while (Poco::Thread::trySleep(t_sleepQueryGetStatus_ms)) {
		...
		for (const auto & subscription : m_subscriptions) {
			...
			if (m_subscriptionIdentifiers.find(subscription.first) == m_subscriptionIdentifiers.end())
				continue;
			...
			if((!t_expireDuration.toDuration_s().first) || (t_expireDuration.toDuration_s().second < t_renewThreshold) || t_expireDuration.toDuration_s().second == std::chrono::seconds(0))
			{
				...
				if (!t_response) {
					log_fatal([&] { return "Renew " + m_subscriptionIdentifiers.at(subscription.first) + " failed."; });
					m_subscriptionIdentifiers.erase(subscription.first);
