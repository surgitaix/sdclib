...
bool ReferenceRange::getRange(Range & out) const {
	if (data->Range().present()) {
		out = ConvertFromCDM::convert(data->Range().get());
		return true;
	}
	return false;
}
...
Range ReferenceRange::getRange() const {
	return ConvertFromCDM::convert(data->Range().get());
}
	
bool ReferenceRange::hasRange() const {
	return data->Range().present();
}
