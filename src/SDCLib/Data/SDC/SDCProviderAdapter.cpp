...
namespace OSELib
{
    ...
    public:
        SetServiceImpl(SDCLib::Data::SDC::SDCProvider& p_provider,
                       const DPWS::MetadataProvider& p_metadata,
        ...
        std::unique_ptr<SDC::SetComponentStateTraits::Response> dispatch(const SDC::SetComponentStateTraits::Request&) override
        {
            //std::lock_guard<std::mutex> t_lock{m_provider.getMutex()}; // FIXME: Mutex should not be used outside of the class!
        ...
        std::unique_ptr<DPWS::RenewTraits::Response> dispatch(const DPWS::RenewTraits::Request& p_request,
                                                              const DPWS::RenewTraits::RequestIdentifier& p_identifier) override
        {
            return m_subscriptionManager.dispatch(p_request, p_identifier);
        }
        std::unique_ptr<DPWS::GetStatusTraits::Response> dispatch(const DPWS::GetStatusTraits::Request& p_request,
                                                                  const DPWS::GetStatusTraits::RequestIdentifier& p_identifier) override
        {
            return m_subscriptionManager.dispatch(p_request, p_identifier);
        }
        std::unique_ptr<DPWS::UnsubscribeTraits::Response> dispatch(const DPWS::UnsubscribeTraits::Request& p_request,
                                                                    const DPWS::UnsubscribeTraits::RequestIdentifier& p_identifier) override
        {
            return m_subscriptionManager.dispatch(p_request, p_identifier);
        }
    };
    ...
    public:
        BICEPSServiceImpl(SDCLib::Data::SDC::SDCProvider& p_provider,
                          const DPWS::MetadataProvider& p_metadata,
                          DPWS::SubscriptionManager& p_subscriptionManager)
            : m_provider(p_provider)
            , m_metadata(p_metadata)
