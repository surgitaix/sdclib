    ...
    def addFunction(self,complexTypeName, abstractBool, requiredProperties_list):
        if not abstractBool:
            ...
            if requiredProperties_list:
                self.__functionDeclarations = self.__functionDeclarations + ", ".join([str(item) for item in requiredProperties_list])
...
class DefaultDefinitionBuilder(object):
    ...
    def addFunction(self, complexTypeName, abstractBool, requiredProperties_list):
        if not abstractBool:
            ...
            if requiredProperties_list:
                self.__functionDefinition = self.__functionDefinition + ", ".join([str(item) for item in requiredProperties_list])
            ...
            if requiredProperties_list:
                ...
                for tupel in requiredProperties_list:
                    requiredPropertiesVar_list.append(tupel.split()[1])
