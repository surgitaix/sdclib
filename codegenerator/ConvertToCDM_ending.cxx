...
std::unique_ptr<typename MdibContainer::WrappedType> ConvertToCDM::convert(const MdibContainer & source) {
	...
	mdibContainer->getMdDescription(convert(source.getMdDescription()));
	mdibContainer->getMdState(convert(source.getMdState()));
	mdibContainer->getMdibVersion(source.getMdibVersion());
