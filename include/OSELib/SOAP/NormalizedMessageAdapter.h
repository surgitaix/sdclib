...
namespace OSELib
{
    namespace SOAP
    {
        ...
        template<>
        struct NormalizedMessageAdapter<DPWS::SubscriptionEndTraits::ReportType>
        {
            const DPWS::SubscriptionEndTraits::ReportType& get(const MESSAGEMODEL::Envelope& message)
            {
                return message.getBody().getSubscriptionEnd().get();
            }

            void set(MESSAGEMODEL::Envelope& message, std::unique_ptr<DPWS::SubscriptionEndTraits::ReportType> source)
            {
                message.getBody().getSubscriptionEnd().set(std::move(source));
            }

            bool present(const MESSAGEMODEL::Envelope& message)
            {
                return message.getBody().getSubscriptionEnd().present();
            }
        };
